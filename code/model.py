...
class Attention(nn.Module):
    def __init__(self):
        ...
        self.sqrt_dim = 1. / torch.sqrt(torch.tensor(args.hidden_dim))  # 用于缩放点积结果的因子，分别基于隐藏维度和特征维度
    ...
    def forward(self, q, k, v, indices, eigs, path_type):
        ni, nx, ny, nz = [], [], [], []
        # ni：存储节点对的索引。nx：存储 q 和 k 点积后的结果。ny：存储特征向量的点积结果。nz：存储路径嵌入的结果。
...
class Model(nn.Module):
    ...
    # 用于评估推荐系统的性能。它在测试数据上计算多种评价指标，如 Precision、Recall 和 NDCG。
    def evaluate(self, test_pos_unique_users, test_pos_list, test_neg_list):
        ...
        with torch.no_grad():
            ...
            for i in range(0, users.shape[0], args.test_batch_size):
                ...
                rating = torch.mm(user_e, item_emb.t())  # 计算当前批次用户对所有物品的评分rating
                for j, u in enumerate(batch_users):
                    # 考虑模型在测试集上的表现，而不受训练集中的信息影响。
    ...
    def valid_func(self):
        return self.evaluate(self.dataset.valid_pos_unique_users, self.dataset.valid_pos_list,
                             self.dataset.valid_neg_list)

    def test_func(self):
        return self.evaluate(self.dataset.test_pos_unique_users, self.dataset.test_pos_list,
                             self.dataset.valid_neg_list)
